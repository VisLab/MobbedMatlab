    ...
    methods
        ...
        function outS = getdb(DB, table, limit, varargin)
            ...
            if ~isempty(parser.Results.inS)
                ...
                nonDoubleStructure = rmfield(structure, doubleColumns);
                doubleStructure = rmfield(structure, ...
                    columns(~ismember(columns, doubleColumns)));
                    [~, doubleValues] = ...
                        DbHandler.extractValues(doubleStructure, ...
                        doubleColumns);
                nonDoubleColumns = columns(ismember(columns, ...
                    doubleColumns));
                nonDoublevalues = DbHandler.createJaggedArray(...
                    struct2cell(nonDoubleStructure));
            end
            outValues = ...
                cell(DB.DbManager.retrieveRows(parser.Results.table, ...
                parser.Results.limit, parser.Results.RegExp, tags, ...
                attributes, nonDoubleColumns, nonDoublevalues, ...
                doubleColumns, doubleValues, parser.Results.DataCursor));
