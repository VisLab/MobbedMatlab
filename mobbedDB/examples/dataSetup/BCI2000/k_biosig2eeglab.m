%% revised bigsig2eeglab() by Kyung
%  - fix the event information import codes
%  - tested with Schalk's BCI dataset (EDF+ format)
...
function EEG = k_biosig2eeglab(dat, DAT, interval, channels, importevent)
...
% wrong event information is deleted. by Kyung, Apr 30, 2012
% EEGLAB v9 and v10 cannot decode the event information of Schalk's edf
% dataset correctly.
% Delete the wrong information and decode the event annotations with my codes.
% version 9: dat.EDF.ANNONS has the event information (1x20000 char format)
%            dat.EVENT.POS is empty.
% version 10: dat.EDFplus.ANNONS has the event information.
%            dat.EVENT.POS is not empty, but filled with wrong event
%            position information.
if ~isempty(dat.EVENT.POS)      % if version 10
    dat.EVENT.POS = [];    
...
if importevent
    ...
    elseif isempty(dat.EVENT.POS)
        ...
        elseif length(unique(EEG.data(end, :))) > 5000
            ...
            % import events code begins. by Kyung
            ...
            % import events code ends. by Kyung

            % begin old code: removed by kyung
%             thiscode = 0;
