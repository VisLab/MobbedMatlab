...
for k = 1:10
    ...
    [UUIDs(k), uniqueEvents] = mat2db(DB, s, true, 'EventTypes', uniqueEvents);
end
...
s.event_entity_uuid = sNew.dataset_uuid;      % search for events from a particular dataset
...
% %% 15.2 Retrieve all of the auxiliary data associated with the primary dataset
% smap = getdb(DB, 'data_maps', 0);    % get an empty data_map template
% smap.data_map_entity_uuid = UUIDs{1};   % find data items mapped to pUUID
