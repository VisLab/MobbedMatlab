...
sdef.data = EEG.data;
sdef.data_def_description = [EEG.setname ' ' EEG.filename ' numeric stream'];
...
load EEG.mat;
sdef = db2data(DB);        
sdef.data_def_format = 'NUMERIC_VALUE';
sdef.data = EEG.data(1,:);
sdef.data_def_description = [EEG.setname ' ' EEG.filename ' numeric'];
UUIDs = data2db(DB, sdef);        
sdef2 = db2data(DB, UUIDs);
assertTrue(isstruct(sdef2));
assertElementsAlmostEqual(sdef.data, sdef2.data);
...
load EEG.mat;
sdef = db2data(DB);        
sdef.data_def_format = 'EXTERNAL';
sdef.data = EEG.data;
sdef.data_def_description = [EEG.setname ' ' EEG.filename ' external'];
UUIDs = data2db(DB, sdef);            
sdef2 = db2data(DB, UUIDs);
assertTrue(isstruct(sdef2));
assertTrue(~isempty(sdef2.data_def_oid));
...
DB = tStruct.DB;
sdef = db2data(DB);         
sdef.data_def_format = 'XML_VALUE';
sdef.data = xmlwrite(which('sample.xml'));
sdef.data_def_description = 'xml';
UUIDs = data2db(DB, sdef);           
sdef2 = db2data(DB, UUIDs);
assertTrue(isequal(sdef.data, sdef2.data));
