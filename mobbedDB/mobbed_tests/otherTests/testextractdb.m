...
function testRange_extractdb(tStruct) %#ok<DEFNU>
fprintf('\nUnit test extractdb function with Range:\n');

fprintf('It should extract all events within default range ([0,1]) with no search qulifications and no limit \n');
DB = tStruct.DB;
[mStructure, extStructure] = extractdb(DB, 'events', [], 'events', [], inf);
assertTrue(~isempty(mStructure));
assertTrue(~isempty(extStructure));

fprintf('It should extract all events within default range ([0,1]) with no search qualifications and a limit\n');
DB = tStruct.DB;
limit = 1;
...
fprintf('It should extract all events within default range ([0,1]) with search qualifications from inStructure and no limit\n');
DB = tStruct.DB;
...
fprintf('It should extract all events within default range ([0,1]) with search qualifications from inStructure and a limit\n');
DB = tStruct.DB;
limit = 1;
% only look for interrelated events in type 1 events 
inStructure.event_type_uuid = tStruct.event_type_uuids{1};
...
fprintf('It should extract all events within default range ([0,1]) with search qualifications from outStructure and no limit\n');
DB = tStruct.DB;
...
fprintf('It should extract all events within default range ([0,1]) with search qualifications from outStructure and a limit\n');
DB = tStruct.DB;
limit = 1;
% only look for type 2 events in all events  
outStructure.event_type_uuid = tStruct.event_type_uuids{2};
...
fprintf('It should extract all events within default range ([0,1]) with search qualifications from inStructure and outStructure and no limit\n');
DB = tStruct.DB;
...
fprintf('It should extract all events within default range ([0,1]) with search qualifications from inStructure and outStructure and a limit\n');
DB = tStruct.DB;
limit = 1;
% only look for type 2 events in type 1 events  
inStructure.event_type_uuid = tStruct.event_type_uuids{1};
...
fprintf('It should extract no events from an event belonging in a different dataset\n');
DB = tStruct.DB; 
